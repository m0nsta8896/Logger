...
class Print:
	...
	def __init__(self, config: PrintConfig):
		...
		Args:
			config (PrintConfig): The configuration object containing logging settings.
		"""
		self.config = config
	...
	def _generic_log(
		...
		if self.config.log_to_console or file:
			self._write_to_console(message, file, color_key, flush)
		if self.config.log_to_file and file is None:
			self._queue.put((message, tag))
	
	def _log_worker(self):
		"""
		The background consumer thread responsible for persistent file logging.
		
		This method runs in a dedicated daemon thread, continuously polling the 
		internal queue for new log entries. It ensures that file I/O operations 
		do not block the main application execution. 
		
		The worker retrieves tuples containing the message and its associated tag, 
		delegating the actual disk write and rotation logic to `_write_to_file`. 
		It continues running until the `_stop_event` is set during shutdown.
		"""
		while not self._stop_event.is_set():
			try:
				msg_data = self._queue.get(timeout=1.0)
				text, tag = msg_data
				self._write_to_file(text, tag)
